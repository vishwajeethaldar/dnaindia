  ...
  return (
    <Stack>
        ...
        <Box>
            ...
            {state.data && state.data.map((news, i)=>{
                  return (
                    i===0?
                    <Box py="15px" px="5px" bg={`light`} mb="10px" borderRadius={`3px` } key={news.id}>
                    <NewsItemCard flexDir={'column'} imgWidth="100%" gap="10px" id={news.id} imgLink={news.image} newsTitle={news.title} path='latestnews' textFont={['1em','1.1em','1.2em']}/>
                    </Box>
                  :
                  <Box py="15px" px="5px" bg={`light`} mb="10px" borderRadius={`3px` } key={news.id}>
